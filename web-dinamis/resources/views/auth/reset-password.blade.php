...
<head>
    ...
    <title>Reset Password - Ade Afwa Boutique</title>
    ...
    <style>
        ...
        .ade-input {
            ...
            background-color: #E5E7EB; /* Abu-abu seperti input pada gambar login Anda */
    ...
    <div class="flex-grow flex items-center justify-center px-4 py-12">
        <div class="max-w-md w-full">
            ...
            <div class="bg-white rounded-lg shadow-2xl p-8 border border-gray-100">
                <h1 class="text-3xl font-serif-ade font-bold text-center text-gray-800 mb-2 uppercase tracking-widest">RESET PASSWORD</h1>
                <p class="text-center text-gray-500 text-sm mb-8">Please enter your new password:</p>
                ...
                <form method="POST" action="{{ route('password.store') }}">
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <input type="email" id="email" name="email" value="{{ old('email', $request->email) }}" required autofocus autocomplete="username" class="ade-input text-sm">
                            <x-input-error :messages="$errors->get('email')" class="mt-2 text-red-600 text-xs" />
                        </div>

                        <div>
                            <label for="password" class="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1">Password</label>
                            <input type="password" id="password" name="password" required autocomplete="new-password" class="ade-input text-sm">
                            <x-input-error :messages="$errors->get('password')" class="mt-2 text-red-600 text-xs" />
                        </div>

                        <div>
                            <label for="password_confirmation" class="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1">Confirm Password</label>
                            <input type="password" id="password_confirmation" name="password_confirmation" required autocomplete="new-password" class="ade-input text-sm">
